...
import { BEHAVIOUR_ANALYSIS } from '../../data/adminMockData'
...
function SpendingChart() {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={BEHAVIOUR_ANALYSIS.spendingDistribution} margin={{ top: 20, right: 10, bottom: 0, left: 0 }}>
        ...
        <Bar dataKey="value" radius={[8, 8, 0, 0]}>
          {BEHAVIOUR_ANALYSIS.spendingDistribution.map((_, i) => (
...
function CategoryPie() {
  return (
    <ResponsiveContainer width="100%" height={280}>
      <PieChart>
        <Pie
          data={BEHAVIOUR_ANALYSIS.favouriteCategories}
          dataKey="value"
          nameKey="name"
          innerRadius={60}
          outerRadius={90}
          ...
          strokeWidth={0}
        >
          {BEHAVIOUR_ANALYSIS.favouriteCategories.map((_, i) => (
            <Cell key={i} fill={COLORS[i % COLORS.length]} />
          ))}
        </Pie>
        <Tooltip
...
function ActivityTimeline() {
  return (
    <ResponsiveContainer width="100%" height={300}>
      <AreaChart data={BEHAVIOUR_ANALYSIS.customerActivity} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
        <defs>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {BEHAVIOUR_ANALYSIS.stats.slice(0, 4).map((s, i) => {
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {BEHAVIOUR_ANALYSIS.stats.slice(4).map((s, i) => {
      ...
      <ChartCard title="Purchase Frequency" subtitle="Orders per customer distribution">
        <div className="grid grid-cols-5 gap-3">
          {BEHAVIOUR_ANALYSIS.purchaseFrequency.map((b, i) => (
      ...
      <ChartCard title="Key Insights" subtitle="AI-generated customer behaviour insights">
        <div className="space-y-3">
          {BEHAVIOUR_ANALYSIS.insights.map((insight, i) => (
