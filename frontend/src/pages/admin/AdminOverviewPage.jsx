...
const SEGMENT_COLORS = SEGMENTATION.segments.map((s) => s.color)
  ...
  return (
    <div className="relative h-64">
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
        <p className="text-2xl font-black text-slate-900">{ADMIN_KPI.totalCustomers.toLocaleString()}</p>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Total Customers" value={ADMIN_KPI.totalCustomers.toLocaleString()} icon={Users} accent="from-red-500 to-orange-500" change="+4.2% vs last month" />
        <KpiCard label="Total Products" value={ADMIN_KPI.totalProducts.toLocaleString()} icon={Package} accent="from-blue-500 to-cyan-500" change="+128 new products" />
        <KpiCard label="Total Orders" value={ADMIN_KPI.totalOrders.toLocaleString()} icon={ShoppingCart} accent="from-emerald-500 to-teal-500" change="+6.4% this month" />
        <KpiCard label="Total Revenue" value="₹3.46Cr" icon={Banknote} accent="from-violet-500 to-purple-500" change="+12.4% growth" />
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Customer Segments" value={ADMIN_KPI.customerSegments} icon={Layers} accent="from-pink-500 to-rose-500" />
        <KpiCard label="Active Campaigns" value={ADMIN_KPI.activeCampaigns} icon={Megaphone} accent="from-amber-500 to-orange-500" />
        <KpiCard label="Recommendation Accuracy" value={`${ADMIN_KPI.recommendationAccuracy}%`} icon={Sparkles} accent="from-indigo-500 to-blue-500" />
        <KpiCard label="Conversion Rate" value={BUSINESS_SUMMARY.conversionRate} icon={TrendingUp} accent="from-teal-500 to-green-500" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <ChartCard title="Business Summary" subtitle="Key metrics overview">
          <div className="space-y-4">
            {Object.entries(BUSINESS_SUMMARY).map(([key, val]) => (
