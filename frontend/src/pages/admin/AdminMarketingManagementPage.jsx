...
import { MARKETING_CAMPAIGNS } from '../../data/adminMockData'
...
export default function AdminMarketingManagementPage() {
  ...
  const filtered = MARKETING_CAMPAIGNS.filter(
    (c) =>
  ...
  const handleCreate = () => {
    ...
    show(`Campaign "${form.name}" created (demo)`, 'success')
    setShowCreate(false)
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">{MARKETING_CAMPAIGNS.length} campaigns total</p>
        </div>
